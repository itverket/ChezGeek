...
namespace Geek2K16.Service.Tests
{
    ...
    public class ChessCalculationsServiceTests
    {
        ...
        [TestMethod]
        public void First_Move_White_Should_Give_MoveHistory_Of_Count_One()
        {
            A?[,] startGrid =
            {
            ...
            var chessBoardState = _chessCalculationsService.GetStateFromGrid(startGrid);
            ...
            var newChessState = _chessCalculationsService.GetStateAfterMove(chessState, move);

            Assert.AreEqual(1, newChessState.MoveHistory.Count);
        }

        [TestMethod]
        public void First_Move_Black_Should_Give_MoveHistory_Of_Count_Two()
        {
            A?[,] startGrid =
            {
                {A.BR, A.BN, A.BB, A.BQ, A.BK, A.BB, A.BN, A.BR},
            ...
            var chessMove = new ChessMove(new ChessPiecePosition(chessPiece, startPosition), endPosition);
            var move = new ExecutedChessMove(chessMove, new TimeSpan());
            chessState = _chessCalculationsService.GetStateAfterMove(chessState, move);

            chessPiece = new ChessPiece(POS.Pawn, Player.Black);
            startPosition = new ChessPosition(COL.E, ROW.Row7);
            endPosition = new ChessPosition(COL.E, ROW.Row5);

            chessMove = new ChessMove(new ChessPiecePosition(chessPiece, startPosition), endPosition);

            move = new ExecutedChessMove(chessMove, new TimeSpan());

            chessState = _chessCalculationsService.GetStateAfterMove(chessState, move);

            Assert.AreEqual(2, chessState.MoveHistory.Count);
        }

        [TestMethod]
        public void First_Move_Black_Should_Give_OpeningState()
        {
            A?[,] startGrid =
            {
                {A.BR, A.BN, A.BB, A.BQ, A.BK, A.BB, A.BN, A.BR},
            ...
            Assert.IsTrue(_chessCalculationsService.IsInOpening(chessState));
        }

        [TestMethod]
        public void FirstMoveWhiteShouldBeAgainstCenter()
        {
            A?[,] startGrid =
            {
                {A.BR, A.BN, A.BB, A.BQ, A.BK, A.BB, A.BN, A.BR},
            ...
            var goodMoves = new List<ChessMove>
            {
                new ChessMove(Player.White, POS.Pawn, COL.E, ROW.Row2, COL.E, ROW.Row4),
                new ChessMove(Player.White, POS.Pawn, COL.D, ROW.Row2, COL.D, ROW.Row4),
                new ChessMove(Player.White, POS.Knight, COL.B, ROW.Row1, COL.C, ROW.Row3)
            };

            Assert.IsTrue(goodMoves.Contains(bestMove.NextMove));
        }
