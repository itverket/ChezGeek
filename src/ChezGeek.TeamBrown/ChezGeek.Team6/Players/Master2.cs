...
namespace ChezGeek.TeamBrown.Players
{
    ...
    public abstract class Master2 : ReceiveActor, IWithUnboundedStash
    {
        private readonly Player _player;
        private readonly IActorRef _randomMoveActor;
        protected CancellationTokenSource _cancellationTokenSource;
        protected readonly FembotChessCalculationsService2 ChessCalculationService;
        protected IActorRef _evaluationRouter;
        private Random _random;

        public IStash Stash { get; set; }

        public Master2(Player player)
        {
            _player = player;
            _cancellationTokenSource = new CancellationTokenSource();
            ChessCalculationService = new FembotChessCalculationsService2();
            _random = new Random();
            Idle();
        }
...
#else
            _evaluationRouter = Context.ActorOf(Props.Create<ChessMoveEvaluatorActor>().WithRouter(new ClusterRouterPool(new RoundRobinPool(40), new ClusterRouterPoolSettings(40, 2, false, "node"))));
#endif
            base.PreStart();
        }
