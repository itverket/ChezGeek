...
namespace ChezGeek.TeamBrown.Services
{
    public class FembotChessCalculationsService2
    {
        ...
        {
            var value = 0f;
            var allPieces = grid.GetAllChessPiecePositions().ToList();

            foreach (var piece in allPieces)
            {
                var pieceValue = Calculations.PieceValues[piece.ChessPiece.PieceType];

                if (piece.ChessPiece.PieceType == PieceType.Knight)
                {
                    if (piece.ChessPosition.Column != ChessColumn.H && piece.ChessPosition.Column != ChessColumn.A)
                    {
                        pieceValue += 0.3f;
                    }
                    if (piece.ChessPosition.Row != ChessRow.Row1 && piece.ChessPosition.Row != ChessRow.Row8)
                    {
                        pieceValue += 0.3f;
                    }
                }

                if (piece.ChessPiece.PieceType == PieceType.Pawn)
                {
                    if (piece.ChessPosition.Column == ChessColumn.E || piece.ChessPosition.Column == ChessColumn.D)
                    {
                        pieceValue += 0.1f;

                        if (piece.ChessPiece.Player == Player.White)
                        {
                            if (piece.ChessPosition.Row == ChessRow.Row4)
                            {
                                pieceValue += 0.2f;
                            }
                        }
                        if (piece.ChessPiece.Player == Player.Black)
                        {
                            if (piece.ChessPosition.Row == ChessRow.Row5)
                            {
                                pieceValue += 0.2f;
                            }
                        }
                ...
                pieceValue *= (int)piece.ChessPiece.Player;

                pieceValue *= allPieces.Count();
