...
namespace ChezGeek.UI
{
    public static class ChessPlayerHelper
    {
        ...
        {
            var assemblies = new List<Assembly> { typeof(ITeamYellowAssembly).Assembly, typeof(OnePlyerActor).Assembly };
        ...
        private static IEnumerable<ChessPlayerModel> GetPlayersFromAssembly(Assembly assembly)
        {
            ...
            return playerActors.Select(x => new ChessPlayerModel { Type = x, Name = x.GetCustomAttribute<ChessPlayerAttribute>().PlayerName });
        }
    }
}