...
$exePath = "$($workingDir)\Geek2k16.Cluster.exe"
$configPath = "$($workingDir)\Geek2k16.Cluster.exe.config"
...
try {
	New-NetFirewallRule `
		-Name "KjessClusterNodeInbound" `
		-DisplayName "Kjess Cluster Node" `
		-Program $exePath `
		-Enabled True `
		-Direction Inbound `
		-Action Allow `
